use crate::{
    config,
    keyboard::keybind::{WCommand, WMouseBind},
};
use x11rb::{
    connection::Connection,
    cursor::Handle as CursorHandle,
    protocol::xproto::{ButtonIndex, ConnectionExt, EventMask, GrabMode},
    ...
    pub fn new<'a, C: Connection>(conn: &'a C, screen_num: usize) -> Self {
        ...
        for bind in &binds {
            let cur = match bind.action {
                WCommand::DragClient => cursors.r#move,
                WCommand::ResizeClient => cursors.resize,
                _ => 0, // invalid
            };
